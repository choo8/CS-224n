...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if max_length - len(sentence) > 0:
            padded_sentence = sentence + ([zero_vector] * (max_length - len(sentence)))
            padded_labels = labels + ([zero_label] * (max_length - len(sentence)))
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        ### END YOUR CODE
        return tf.reshape(tf.nn.embedding_lookup(embeddings, self.input_placeholder), shape=(-1, self.max_length, self.config.n_features * self.config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o, h = cell(tf.squeeze(x[:,time_step,:]), h, scope="RNN")
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        loss = tf.reduce_mean(tf.boolean_mask(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds), mask=self.mask_placeholder))
        ### END YOUR CODE
        return loss
