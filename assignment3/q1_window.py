...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        augmented_sentence = ([start] * window_size) + sentence + ([end] * window_size)

        for idx, word in enumerate(sentence):
            shifted_idx = idx + window_size
            cur_window = []
            for i in range(shifted_idx - window_size, shifted_idx + window_size + 1):
                cur_window.extend(augmented_sentence[i])
            windowed_data.append((cur_window, labels[idx]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        ### END YOUR CODE
        return tf.reshape(tf.nn.embedding_lookup(embeddings, self.input_placeholder), [-1, self.config.n_window_features * self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(initializer((self.config.n_window_features * self.config.embed_size, self.config.hidden_size)))
